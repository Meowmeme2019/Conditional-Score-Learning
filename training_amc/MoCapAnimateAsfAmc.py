...
class MotionCapture(object):
    ...
    def Animate(self, limbs=None):
        """
        Animate 3D joint positions using matplotlib.
        limbs: list of (parent_id, child_id) pairs to draw bones
        """
        if limbs is None:
            limbs = self.limbs
        ...
        def update(frame):
            for i, (a, b) in enumerate(limbs):
                x = [self.data[frame, a, 0], self.data[frame, b, 0]]
                y = [self.data[frame, a, 1], self.data[frame, b, 1]]
                z = [self.data[frame, a, 2], self.data[frame, b, 2]]
        ...
        ani = animation.FuncAnimation(
            fig, update, frames=self.num_frames,
            init_func=init, interval=20, blit=False  # TURN OFF BLIT
        )
